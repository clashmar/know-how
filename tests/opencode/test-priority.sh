...
# Verifies that skills are resolved with correct priority: project > personal > bundled plugin
...
# 1. Create in bundled plugin location (lowest priority)
mkdir -p "$SKILLS_DIR/priority-test"
cat > "$SKILLS_DIR/priority-test/SKILL.md" <<'EOF'
---
name: priority-test
description: Bundled version of priority test skill
---
# Priority Test Skill (Bundled Version)

This is the bundled plugin version of the priority test skill.

PRIORITY_MARKER_BUNDLED_VERSION
EOF
...
if [ -f "$SKILLS_DIR/priority-test/SKILL.md" ]; then
    echo "  [PASS] Bundled version exists"
else
    echo "  [FAIL] Bundled version missing"
    exit 1
fi
...
# Test 2: Test that personal overrides bundled skill
echo ""
echo "Test 2: Testing personal > bundled priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [PASS] Personal version loaded (overrides bundled skill)"
elif echo "$output" | grep -qi "PRIORITY_MARKER_BUNDLED_VERSION"; then
    echo "  [FAIL] Bundled version loaded instead of personal"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
    echo "$output" | grep -i "priority\|personal\|bundled" | head -10
fi

# Test 3: Test that project overrides both personal and bundled skill
echo ""
echo "Test 3: Testing project > personal > bundled priority..."
...
if echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION"; then
    ...
    echo "  [FAIL] Bundled version loaded instead of project"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
    echo "  Output snippet:"
...
# Test 4: Test explicit know-how: prefix bypasses priority
echo ""
echo "Test 4: Testing know-how: prefix forces bundled version..."

cd "$TEST_HOME/test-project"
output=$(timeout 60s opencode run --print-logs "Use the use_skill tool to load know-how:priority-test specifically. Show me the exact content including any PRIORITY_MARKER text." 2>&1) || {
    exit_code=$?
    if [ $exit_code -eq 124 ]; then
        echo "  [FAIL] OpenCode timed out after 60s"
        exit 1
...
if echo "$output" | grep -qi "PRIORITY_MARKER_BUNDLED_VERSION"; then
    echo "  [PASS] know-how: prefix correctly forces bundled version"
elif echo "$output" | grep -qi "PRIORITY_MARKER_PROJECT_VERSION\|PRIORITY_MARKER_PERSONAL_VERSION"; then
    echo "  [FAIL] know-how: prefix did not force bundled version"
    exit 1
else
    echo "  [WARN] Could not verify priority marker in output"
fi
