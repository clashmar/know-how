...
- **Always include one scout that covers file structure.** When identifying scout domains, ensure one scout surveys the existing directory layout and file naming conventions. Its questions must cover: (1) what naming conventions are used for analogous files, (2) which directories own which types of concerns, (3) where the closest existing equivalents to the planned new files currently live. This informs placement decisions during design.
