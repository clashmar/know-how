...
6. **Write the design doc** — dispatch the **deckbuilder** agent to render the spec as
   HTML. Pass the validated spec as a JSON payload (see `deckbuilder-example.json` for
   format) with output path `~/.know-how/<project-name>/specs/YYYY-MM-DD-<topic>.html`.
   The deckbuilder returns a `file://` link — present this link to the user.
...
- Dispatch the **deckbuilder** agent with the validated spec as a JSON payload
  (see `deckbuilder-example.json` for format) and output path
